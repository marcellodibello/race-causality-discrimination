...
the Rubin's model for studying causality.^[Donald Rubin is an Emeritus Professor of Statistics at Harvard University, known for the Rubin Causal Model.]  This is a popular framework that describes how many
...
\[Y_t(u)-Y_c(u)\]
This difference can hardly be directly observed because the same unit $u$ cannot be both subject to treatement and control (at the same time).^[See the fundamental problem of causal inference, p. 947, also discussed later.]. 
...
> Show that the independence assumption and constant effect assumption, toghether, solve the fundamental problem of causal inference? [Hint: show that under these assumptions $T=T_{PF}$.]

> Show that the constant effect assumption, alone, is not enough to guaranteert that $T=T_{PF}$
...
The philosopher Patrick Suppes proposed the 
...
to illustrate a  disagreement between Fisher and Neyman: 

> Explain the disagreement between Fisher and Neyman about the choice of the null hypothesis 
in experiments with randomized bloack design? 
...
>  One may view Fisher's (1957) attack on those who used
 the association between smoking and lung cancer as evi-
 dence of a "causal link" between them as an example of
